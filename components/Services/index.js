...
const Services = () => {
    const [showServiceModal, setShowServiceModal] = useGlobalState('serviceModal');
    const [modalType, setModalType] = useState(null);
    const handleOnClick = (e) => {
        if (showServiceModal && e === modalType) {
        ...
        <div className='mt-28'>
            ...
            <div className='grid grid-flow-col gap-28'>
                <div className='grid grid-flow-row text-center border border-tealGreen rounded border-l-0 bg-darkGrey'>
                    ...
                    <div
                        className='w-full flex justify-center rounded-b bg-gradient-to-r from-zinc-900 via-zinc-900 to-opaque'
                        onClick= { () => handleOnClick('construction')} >
                        <IconLoader xlinkHref= { 'chevron-down' } className='h-7 w-5' onClick= { () => handleOnClick('construction')} />
                    </div>
                </div>
                <div className='grid grid-flow-row text-center border border-tealGreen rounded border-r-0 bg-darkGrey'>
                    ...
                    <div
                        className='w-full flex justify-center rounded-b bg-gradient-to-l from-zinc-900 via-zinc-900 to-opaque'
                        onClick= { () => handleOnClick('maintenance')} >
